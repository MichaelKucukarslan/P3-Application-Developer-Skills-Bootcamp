...
class RoundController(BaseScreen):
    ...
    It first creates the next round.
    Then allows the user to input winners for that round.
    """
    ...
    def get_command(self):
        ...
        print(
            ...
            f"Number of rounds: {self.tournament.number_of_rounds}"
        )
        while (
            ...
            if (
                ...
                round_view.print_completed_tournament(
                    self.tournament.get_players_with_points()
                    )
                self.tournament.save()
                break
